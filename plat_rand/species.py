"""National Dex 1-493 names and randomization pools."""
...
def species_pool(*, allow_legendaries: bool = True) -> list[int]:
    pool = list(range(1, 494))
    if not allow_legendaries:
        pool = [sid for sid in pool if sid not in LEGENDARIES]
    return pool
